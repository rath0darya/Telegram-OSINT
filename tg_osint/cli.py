...
def main():
    ...
    Path(a.out).mkdir(parents=True,exist_ok=True); db=CaseDB(a.db); cid=db.create_case(t["handle"]); ev=[]; errors=[]
    ...
    report=build_report(t,cid,ev,errors,analysis); hp=str(Path(a.out)/f"{t['username']}_{cid}.html"); write_html(report,hp); db.close()
