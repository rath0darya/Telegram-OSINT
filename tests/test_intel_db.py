...
class IntelligenceDBTests(unittest.TestCase):
    ...
    def test_username_history_stays_on_same_telegram_id(self):
        ...
        try:
            for username, ts in (
                ("old_name", "2026-09-18T00:00:00+00:00"),
                ("new_name", "2026-09-19T00:00:00+00:00"),
            ):
                ev = Evidence(
                    ...
                    username,
                    username,
                    sha256_text(username + ts),
                    {"entity": {"id": 123, "username": username, "first_name": "Test"}},
                )
                db.ingest([ev])
            rows = db.search("123")
            ...
            self.assertEqual(
                [x["value"] for x in history["identifiers"] if x["identifier_type"] == "telegram_username"],
                ["old_name", "new_name"],
            )
    ...
    def test_username_reuse_never_merges_different_telegram_ids(self):
        ...
        try:
            ...
            rows = db._execute(
                "SELECT entities.telegram_id, profile_snapshots.first_name "
                "FROM profile_snapshots JOIN entities ON entities.id=profile_snapshots.entity_id "
        ...
        try:
            ...
            cols = {
                ...
                for r in db._execute(
                    ...
                    "WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='edges'"
                ).fetchall()
            }
